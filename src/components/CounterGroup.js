...
const CounterGroup = ({ groupId, group }) => {
	...
	const handleAddNewCounter = () => {
		dispatchData({ type: 'ADD_COUNTER', payload: { groupId, id: RandomID(), newCounter: newCounterMetadata } });
		console.log(groupId);
	};
	...
	return (
		...
		<div className="CounterGroup bg-light py-2 px-5 m-2 rounded">
			...
			{ Object.keys(group.counters).map(counterId => (
				<Counter key={ counterId } id={ counterId } options={ counterOptions } counter={ group.counters[counterId] } deleteCounter={ deleteCounter } />
			)) }
		</div>
